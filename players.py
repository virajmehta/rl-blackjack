'''This is the file containing all the code needed for each type of blackjack player. It will eventually contain six player types: Dealer, Baseline, Oracle, Hi-Lo, Omega II, and Wong Halves. It currently contains the implementations for the first three.''' 
...
class Player():
    ...
    def getWinnings(self):
        '''Calls getReward() from Game to retrieve the outcome of the hand. Updates total winnings over the whole game.'''
...
class Dealer():
    '''The dealer's implementation. Since the dealer's actions are taken care of in the __endHand__ method from Game, this simply outputs the dealer's final hand and total.'''
...
class Oracle():
    '''The oracle implementation. This player evaluates all possible actions given the hand and peeks into the deck to make the best possible move for each state, i.e. the perfect blackjack player.'''
    def __init__(self, game):
        self.game = game
        self.hand = []
        self.total = 0
    ...
    def standValue(self):
        '''Future value of standing on the given hand. Calls peekDealer() to see how the hand will compare to the dealer's.'''
        return self.peekDealer(False)

    def hitValue(self):
        '''Future value of hitting on the given hand. Returns the negative bet if the player will bust, otherwise returns 0.'''
    ...
    def doubleValue(self):
        '''Future value of doubling down on the given hand. Calls peekDealer() to see how the hand will compare to the dealer's and returns the double the reward.'''
    ...
    def peekDealer(self, doubling):
        '''Allows oracle to see into the future and peek at the hand that the dealer will have after taking a given action (either stand or double). Uses doubling flag to indicate where in the deck to start peeking. Returns future reward based on the comparison of player and dealer hands.'''
    ...
    def getDealerTotal(self, newIndex):
        '''Plays out the dealer's hand and returns what the dealer's final total would be. Based on __endHand__ from Game.'''
    ...
    def peekReward(self, playerTotal, dealerTotal):
        '''Returns what the player's reward would be based on the comparison of the two hands. Based on getReward() from Game.'''
        if playerTotal > 21:
            return -1 * self.game.bet
        if dealerTotal > 21:
